...
define(['jquery'],factory);}else if(_typeof(exports)==='object'){// CommonJS
...
$.cookie(key,'',$.extend({},options,{expires:-1}));return!$.cookie(key);};});},{"jquery":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\jquery\\dist\\jquery.js"}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\jquery\\dist\\jquery.js":[function(require,module,exports){/*!
...
if(!noGlobal){window.jQuery=window.$=jQuery;}return jQuery;});},{}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\slick-carousel\\slick\\slick.js":[function(require,module,exports){/*
     _ _      _       _
 ___| (_) ___| | __  (_)___
/ __| | |/ __| |/ /  | / __|
\__ \ | | (__|   < _ | \__ \
...
p.attack=50;p.hold=50;p.decay=100;p.initValue=0;p.holdValue=particleAlpha;p.lastValue=0;//the particle will be stuck in one place until this time has elapsed:
p.stuckTime=90+Math.random()*20;p.accelX=0;p.accelY=gravity;p.accelZ=0;}}//update viewing angle
turnAngle=(turnAngle+turnSpeed)%(2*Math.PI);sinAngle=Math.sin(turnAngle);cosAngle=Math.cos(turnAngle);//background fill
context.fillStyle="#151C28";context.fillRect(0,0,displayWidth,displayHeight);//update and draw particles
p=particleList.first;while(p!=null){//before list is altered record next particle
nextParticle=p.next;//update age
p.age++;//if the particle is past its "stuck" time, it will begin to move.
if(p.age>p.stuckTime){p.velX+=p.accelX+randAccelX*(Math.random()*2-1);p.velY+=p.accelY+randAccelY*(Math.random()*2-1);p.velZ+=p.accelZ+randAccelZ*(Math.random()*2-1);p.x+=p.velX;p.y+=p.velY;p.z+=p.velZ;}/*
      We are doing two things here to calculate display coordinates.
      The whole display is being rotated around a vertical axis, so we first calculate rotated coordinates for
      x and z (but the y coordinate will not change).
      Then, we take the new coordinates (rotX, y, rotZ), and project these onto the 2D view plane.
      */rotX=cosAngle*p.x+sinAngle*(p.z-sphereCenterZ);rotZ=-sinAngle*p.x+cosAngle*(p.z-sphereCenterZ)+sphereCenterZ;m=radius_sp*fLen/(fLen-rotZ);p.projX=rotX*m+projCenterX;p.projY=p.y*m+projCenterY;//update alpha according to envelope parameters.
if(p.age<p.attack+p.hold+p.decay){if(p.age<p.attack){p.alpha=(p.holdValue-p.initValue)/p.attack*p.age+p.initValue;}else if(p.age<p.attack+p.hold){p.alpha=p.holdValue;}else if(p.age<p.attack+p.hold+p.decay){p.alpha=(p.lastValue-p.holdValue)/p.decay*(p.age-p.attack-p.hold)+p.holdValue;}}else{p.dead=true;}//see if the particle is still within the viewable range.
if(p.projX>displayWidth||p.projX<0||p.projY<0||p.projY>displayHeight||rotZ>zMax){outsideTest=true;}else{outsideTest=false;}if(outsideTest||p.dead){recycle(p);}else{//depth-dependent darkening
depthAlphaFactor=1-rotZ/zeroAlphaDepth;depthAlphaFactor=depthAlphaFactor>1?1:depthAlphaFactor<0?0:depthAlphaFactor;context.fillStyle=rgbString+depthAlphaFactor*p.alpha+")";//draw
context.beginPath();context.arc(p.projX,p.projY,m*particleRad,0,2*Math.PI,false);context.closePath();context.fill();}p=nextParticle;}}function addParticle(x0,y0,z0,vx0,vy0,vz0){var newParticle;var color;//check recycle bin for available drop:
if(recycleBin.first!=null){newParticle=recycleBin.first;//remove from bin
if(newParticle.next!=null){recycleBin.first=newParticle.next;newParticle.next.prev=null;}else{recycleBin.first=null;}}//if the recycle bin is empty, create a new particle (a new ampty object):
else{newParticle={};}//add to beginning of particle list
if(particleList.first==null){particleList.first=newParticle;newParticle.prev=null;newParticle.next=null;}else{newParticle.next=particleList.first;particleList.first.prev=newParticle;particleList.first=newParticle;newParticle.prev=null;}//initialize
newParticle.x=x0;newParticle.y=y0;newParticle.z=z0;newParticle.velX=vx0;newParticle.velY=vy0;newParticle.velZ=vz0;newParticle.age=0;newParticle.dead=false;if(Math.random()<0.5){newParticle.right=true;}else{newParticle.right=false;}return newParticle;}function recycle(p){//remove from particleList
if(particleList.first==p){if(p.next!=null){p.next.prev=null;particleList.first=p.next;}else{particleList.first=null;}}else{if(p.next==null){p.prev.next=null;}else{p.prev.next=p.next;p.next.prev=p.prev;}}//add to recycle bin
if(recycleBin.first==null){recycleBin.first=p;p.prev=null;p.next=null;}else{p.next=recycleBin.first;recycleBin.first.prev=p;recycleBin.first=p;p.prev=null;}}}},{}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\jquery-global.js":[function(require,module,exports){"use strict";var _jquery=_interopRequireDefault(require("jquery"));function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{"default":obj};}window.jQuery=_jquery["default"];window.$=_jquery["default"];},{"jquery":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\jquery\\dist\\jquery.js"}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\main.js":[function(require,module,exports){'use strict';require("./jquery-global.js");require("jquery.cookie");require("slick-carousel");require("./tabs");require("./burger");require("./hero-canvas");require("./progress-bar");},{"./burger":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\burger.js","./hero-canvas":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\hero-canvas.js","./jquery-global.js":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\jquery-global.js","./progress-bar":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\progress-bar.js","./tabs":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\tabs.js","jquery.cookie":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\jquery.cookie\\jquery.cookie.js","slick-carousel":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\node_modules\\slick-carousel\\slick\\slick.js"}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\progress-bar.js":[function(require,module,exports){"use strict";Object.defineProperty(exports,"__esModule",{value:true});exports.changeLineWidth=changeLineWidth;var $proggressLine=$('.js-progress-line');var $activeTab=$('.js-tab.is-active');var $currentTabIndex=$('.js-current-tab-index');var $lastTabIndex=$('.js-last-tab-index');changeLineWidth($activeTab);function changeLineWidth($activeTab){var $this=$activeTab;var currentIndex=$this.data('tab');var lastIndex=$this.data('tab-end');var progressLineWidthPercents=currentIndex/lastIndex;$currentTabIndex.html(currentIndex);$lastTabIndex.html(lastIndex);$proggressLine.css('transform',"scaleX(".concat(progressLineWidthPercents,")"));}},{}],"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\tabs.js":[function(require,module,exports){"use strict";var _progressBar=require("./progress-bar");var $tab=$('.js-tab');var $tabContent=$('.js-tab-content');var $tabInner=$('.js-tab-inner');var $tabContentInner=$('.js-tab-content-inner');$tab.on('click',toggleTabContent);$tabInner.on('click',toggleInnerTabContent);function toggleTabContent(){var tabToShow=$(this).data('tab');$tab.removeClass('is-active');$tabContent.removeClass('is-active');$(this).addClass('is-active');$("#".concat(tabToShow)).addClass('is-active');(0,_progressBar.changeLineWidth)($(this));}function toggleInnerTabContent(){var tabToShow=$(this).data('tab');$tabInner.removeClass('is-active');$tabContentInner.removeClass('is-active');$(this).addClass('is-active');$("#".concat(tabToShow)).addClass('is-active');}},{"./progress-bar":"C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\progress-bar.js"}]},{},["C:\\Users\\\u041A\u043E\u043B\u044F\\Desktop\\Dominor\\src\\js\\main.js"]);
